...
pgamma(q = 1, shape = gamma_shape, scale = secondary_mean/gamma_shape) 


gen_int <- tibble(
  x = seq(0, 40, 0.01),
  y = dgamma(x = x, shape = gamma_shape, scale = secondary_mean / gamma_shape)
)
...
ggplot(gen_int, aes(x = x, y = y)) + 
  geom_area(colour = "seagreen4", fill = "seagreen4", alpha = 0.5) + 
  coord_cartesian(xlim = c(0, 20)) + 
  scale_y_continuous(labels = scales::percent_format(accuracy = 1), minor_breaks = NULL) + 
  labs(y = "Density", x = "Generation Interval (days)") + 
  theme_classic()

save_plot("params_generation_interval_desc_paper")
